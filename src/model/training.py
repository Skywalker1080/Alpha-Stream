...
def fit_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 10, device: str = Config().device, lr: float = 1e-3) -> nn.Module:
    try:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            logger.info(f"MODEL TRAINING - Epoch {epoch}/{num_epochs} - Train Loss: {avg_train_loss:.5f}")
            ...
            logger.info(f"MODEL TRAINING - Epoch {epoch}/{num_epochs} - Validation Loss: {avg_val_loss:.5f}")
            ...
            if avg_val_loss < best_val_loss:
                ...
                counter += 1
                if counter >= patience:
                    logger.info("MODEL TRAINING - Early stopping triggered")
                    break

        return model
    except PrismException as e:
        logger.exception(f"MODEL TRAINING - Failed to train model: {e}")
