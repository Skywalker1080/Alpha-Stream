...
`train_parent()` / `train_child(ticker)` — with TimesFM this is a **zero-shot provisioning step**, not weight training: fetch OHLCV → `StandardScaler` fit → load TimesFM → `evaluate_quick()` (single-window validation, one forward pass, ~seconds not minutes) → save the per-ticker scaler → log metrics/artifacts to MLflow. Artifacts land at `outputs/{ticker}/{ticker}_child_scaler.pkl` (parent: `outputs/parent/{parent_ticker}_parent_scaler.pkl`). Whether a model "exists" for a ticker = that scaler exists (see `ModelProvisioner`, `src/model/provisioning.py`). The legacy `.pt` weights and `CryptoData`/`fit_model` path are dead. The legacy full-history backtest lives on as `evaluate_model_temp()` (unused by the running pipeline). Metrics are computed on OHLC only — Volume's raw-unit magnitude would swamp MSE/RMSE.
